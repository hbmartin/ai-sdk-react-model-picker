...
export function ModelProviderTag({ tag, className = '' }: ModelProviderTagProps) {
  ...
  const getTagClasses = () => {
    if (environment === 'vscode') {
      ...
      return {
        [ModelProviderTags.RequiresApiKey]: 'bg-vscode-error/20 text-vscode-error border border-vscode-error/30',
        [ModelProviderTags.Local]: 'bg-vscode-success/20 text-vscode-success border border-vscode-success/30',
        [ModelProviderTags.Free]: 'bg-vscode-warning/20 text-vscode-warning border border-vscode-warning/30',
        [ModelProviderTags.OpenSource]: 'bg-vscode-button-bg/20 text-vscode-button-fg border border-vscode-button-bg/30',
        [ModelProviderTags.Vision]: 'bg-vscode-text-link/20 text-vscode-text-link border border-vscode-text-link/30',
        [ModelProviderTags.Tools]: 'bg-vscode-info/20 text-vscode-info border border-vscode-info/30',
        [ModelProviderTags.LongContext]: 'bg-vscode-warning/15 text-vscode-warning border border-vscode-warning/25',
      };
    } else {
      // Standard web environment using semantic color classes
      return {
        [ModelProviderTags.RequiresApiKey]: 'bg-destructive/10 text-destructive border border-destructive/20',
  ...
  const tagClasses = getTagClasses()[tag] || 'bg-accent text-accent-foreground border border-border';
