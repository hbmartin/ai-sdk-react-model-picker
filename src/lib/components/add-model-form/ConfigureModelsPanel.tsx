...
export function ConfigureModelsPanel({
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="mt-4 flex-1 overflow-y-auto rounded border border-border bg-background">
          ...
          <>
            ...
            <ul className="space-y-2 ps-0">
              {providerModels.map((entry) => {
                ...
                const checkboxChecked = entry.model.visible ?? true;
                const isDisabled = pendingVisibilityUpdate === entry.model.id;
                return (
                  <li
                    key={entry.key}
                    className={`flex items-center justify-between gap-2 rounded border py-2 text-sm transition-colors ${
                      isHighlighted
        ...
        {providerModelsStatus?.error !== undefined && (
